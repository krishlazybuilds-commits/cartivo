...
class GoogleLoginView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger.warning("Google login CSRF check failed: %s", exc)
            raise
        ...
        try:
            ...
            logger.warning("Google login user lookup/creation failed: %s", exc.detail)
            return Response({"detail": exc.detail}, status=status.HTTP_409_CONFLICT)

        if not user.is_active:
            logger.warning("Google login rejected — user pk=%s is disabled", user.pk)
            return Response(
        ...
        refresh = RefreshToken.for_user(user)
        logger.info("Google login successful for user pk=%s", user.pk)
